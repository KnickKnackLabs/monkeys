# monkeys

<img src="logo.jpg" alt="monkeys logo" width="256">

*A monkey playing drums — generated by [@johnson-ricon](https://github.com/johnson-ricon) via `monkeys generate`*

CLI for generating media — images, speech, transcription, and OCR.

## Install

```bash
git clone https://github.com/KnickKnackLabs/monkeys.git ~/monkeys
cd ~/monkeys && mise trust && mise install

# Add to your shell config (~/.zshrc or ~/.bashrc)
eval "$(mise -C ~/monkeys run -q shell)"

# Reload and verify
source ~/.zshrc
monkeys welcome
```

## Configuration

Set `HF_TOKEN` for Hugging Face API authentication:

```bash
export HF_TOKEN="hf_your_token_here"
```

Get a token at https://huggingface.co/settings/tokens

## Usage

```bash
# Image generation (requires HF_TOKEN)
monkeys generate "a cartoon monkey"
monkeys generate "pixel art sunset" -o sunset.png

# Text-to-speech (local, no token needed)
monkeys speak "Hello world"
monkeys speak "Good morning" --voice af_bella --play

# Speech-to-text (local, requires ffmpeg)
monkeys listen recording.wav
monkeys listen recording.wav -o transcript.txt
monkeys listen --record -d 5              # Record from mic for 5 seconds
monkeys listen --record                   # Record until Ctrl+C

# OCR — extract text and coordinates from images (local, no token needed)
monkeys ocr screenshot.png                    # JSON with text + bounding boxes
monkeys ocr screenshot.png --find "Select"    # Filter by substring
monkeys ocr screenshot.png --text-only        # Plain text output

monkeys welcome   # Verify setup
monkeys tasks     # List all commands
```

## Development

This project uses [shimmer](https://github.com/KnickKnackLabs/shimmer) for agent workflows.
